...
std::list<std::string> ZBZDControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBZDControlDialog::listMints;

ZBZDControlDialog::ZBZDControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBZDControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBZDControlDialog::~ZBZDControlDialog()
{
    delete ui;
}

void ZBZDControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBZDControlDialog::updateList()
{
...
void ZBZDControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBZDControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBZD_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBZDControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBZDControlDialog::GetSelectedMints()
{
...
void ZBZDControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
