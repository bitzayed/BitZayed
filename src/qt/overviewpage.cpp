...
// Copyright (c) 2018 The BitZayed developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BZD)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sBZDPercentage, QString& szBZDPercentage)
{
    int nPrecision = 2;
    ...
    szBZDPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sBZDPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount bzdAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = bzdAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // BZD labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, bzdAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelBZDPercent->setText(sPercentage);
    ui->labelzBZDPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zBZD.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in bitzayed.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in bitzayed.conf");
    }
    ...
    bool showBZDAvailable = settingShowAllBalances || bzdAvailableBalance != nTotalBalance;
    bool showWatchOnlyBZDAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showBZDPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyBZDPending = watchUnconfBalance != 0;
    bool showBZDLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyBZDLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showBZDAvailable || showWatchOnlyBZDAvailable);
    ui->labelBalanceText->setVisible(showBZDAvailable || showWatchOnlyBZDAvailable);
    ui->labelWatchAvailable->setVisible(showBZDAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showBZDPending || showWatchOnlyBZDPending);
    ui->labelPendingText->setVisible(showBZDPending || showWatchOnlyBZDPending);
    ui->labelWatchPending->setVisible(showBZDPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showBZDLocked || showWatchOnlyBZDLocked);
    ui->labelLockedBalanceText->setVisible(showBZDLocked || showWatchOnlyBZDLocked);
    ui->labelWatchLocked->setVisible(showBZDLocked && showWatchOnly);
    ...
    bool showzBZDAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzBZDUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzBZDImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzBZDAvailable);
    ui->labelzBalanceMatureText->setVisible(showzBZDAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzBZDUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzBZDUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzBZDImmature);
    ui->labelzBalanceImmatureText->setVisible(showzBZDImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelBZDPercent->setVisible(showPercentages);
    ui->labelzBZDPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BZD")
    updateDisplayUnit();
}
