...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BZD);
    ...
    if (!settings.contains("nAnonymizeBitZayedAmount"))
        settings.setValue("nAnonymizeBitZayedAmount", 1000);

    nAnonymizeBitZayedAmount = settings.value("nAnonymizeBitZayedAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBitZayedAmount"))
        SoftSetArg("-anonymizebitzayedamount", settings.value("nAnonymizeBitZayedAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitzayed.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitZayedAmount:
            return QVariant(nAnonymizeBitZayedAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitZayedAmount:
            nAnonymizeBitZayedAmount = value.toInt();
            settings.setValue("nAnonymizeBitZayedAmount", nAnonymizeBitZayedAmount);
            emit anonymizeBitZayedAmountChanged(nAnonymizeBitZayedAmount);
            break;
        case CoinControlFeatures:
