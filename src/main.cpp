...
#if defined(NDEBUG)
#error "BitZayed cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubzd) are considered zero fee (for relaying and mining)
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zBZD with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zBZD spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zBZD spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
{
	const int PRE_BLOCKCOUNT = 2500;
	const int FIRST_BLOCKCOUNT = 2500;
	const int YEARLY_BLOCKCOUNT = 100000;
    int64_t nSubsidy = 0;
    ...
    // Don't sync this code, it's specified by BZD
	if (nHeight <= 10) {
        nSubsidy = 50000 * COIN;
	} else if (nHeight <= PRE_BLOCKCOUNT && nHeight > 10) {
        nSubsidy = 2 * COIN;
	} else if (nHeight <= FIRST_BLOCKCOUNT && nHeight > PRE_BLOCKCOUNT) {
        nSubsidy = 30 * COIN;
	} else if (nHeight <= (FIRST_BLOCKCOUNT * 1.5) && nHeight > FIRST_BLOCKCOUNT) {
        nSubsidy = 25 * COIN;
	} else if (nHeight <= (FIRST_BLOCKCOUNT * 2) && nHeight > (FIRST_BLOCKCOUNT * 1.5)) {
        nSubsidy = 20 * COIN;
	} else if (nHeight <= (FIRST_BLOCKCOUNT * 3) && nHeight > (FIRST_BLOCKCOUNT * 2)) {
        nSubsidy = 15 * COIN;
	} else if (nHeight <= (FIRST_BLOCKCOUNT * 4) && nHeight > (FIRST_BLOCKCOUNT * 3)) {
        nSubsidy = 12.5 * COIN;
    } else {
	nSubsidy = 12.5 * COIN;
	}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from BitZayed
...
{
    RenameThread("bitzayed-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZBZDMinted()
{
...
void RecalculateZBZDSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zBZD supply
...
bool RecalculateBZDSupply(int nHeightStart)
{
...
bool UpdateZBZDSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateBZDSupply(Params().Zerocoin_StartHeight());
    }

    //Track zBZD money supply in the block index
    if (!UpdateZBZDSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zBZD supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zBZDSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BZD to zBZD
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBZD spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBZD serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zBZD mints and %d zBZD spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BitZayed: We use certain sporks during IBD, so check to see if they are
