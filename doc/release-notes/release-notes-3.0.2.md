BitZayed Core version 3.0.2 is now available from:
...
BitZayed Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zBZD spends, automint calculation adjustments, and other various updates/fixes.

zBZD spending requires this update.
...
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zBZD mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

zBZD Spending Fix
---------------------
The size of zBZD spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

zBZD Transaction Recovery
---------------------
Due to the aforementioned issue with zBZD spending, users may find that their attempted spends are now conflicted and zBZD balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the BitZayed address as it's first parameter and the passphrase as it's second parameter.
...
- #292 `feadab4` Additional checks for double spending of zBZD serials. (presstab)

### Wallet
- #271 `5e9a086` [Wallet] Remove unused member wallet in UnlockContext inner class (Jon Spock)
- #279 `e734010` Add -backupzbzd startup flag. (presstab)
- #280 `fdc182d` [Wallet] Fix zBZD spending errors. (presstab)
- #282 `310f216` [Wallet] Count pending zBZD balance for automint. (presstab)
...
- #291 `cb314e6` [Qt] zBZD control quantity/amount fixes (rejectedpromise)
